...
public class VulkanBasicRendering {
    ...
    public void drawFrame() {
        ...
        } catch (Vulkan.BufferRecordException e) {
        ...
        pCommandBuffers.put(context.vulkan.getCurrentCommandBuffer());

        VkSubmitInfo submitInfo = VkSubmitInfo.calloc(stack);
        submitInfo.sType(VK_STRUCTURE_TYPE_SUBMIT_INFO);
        submitInfo.waitSemaphoreCount(1);
        submitInfo.pWaitSemaphores(RenderUtils.store(waitSemaphores));
        submitInfo.pWaitDstStageMask(RenderUtils.store(waitStages));
        submitInfo.pCommandBuffers(pCommandBuffers);
        submitInfo.pSignalSemaphores(RenderUtils.store(signalSemaphores));
        ...
        presentInfo.pSwapchains(RenderUtils.store(swapChains));
